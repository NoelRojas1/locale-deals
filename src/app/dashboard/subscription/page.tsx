          ...
          <Card>
            <CardHeader>
              <CardTitle>
                ...
                <span className="text-accent">{tier.name}</span> plan
              </CardTitle>
              <CardDescription>
  ...
  return (
    <Card className="shadow-none rounded-3xl overflow-hidden">
      ...
      <CardFooter className="flex flex-col gap-4 items-start">
        <Feature className="text-bold">
          {maxNumberOfProducts}{" "}
          {maxNumberOfProducts === 1 ? "product" : "products"}
        </Feature>

        <Feature>Locale Deals discounts</Feature>
        {canCustomizeBanner && <Feature>Banner Customization</Feature>}
        {canAccessAnalytics && <Feature>Advanced analytics</Feature>}
        {canRemoveBranding && <Feature>Remove Locale Deals branding</Feature>}
      </CardFooter>
    </Card>
  );
}
