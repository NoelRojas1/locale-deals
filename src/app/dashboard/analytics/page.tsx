...
import ViewsByDealsChart from "../_components/charts/ViewsByDealsChart";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
        ...
        <HasPermission permission={canAccessAnalytics}>
          <div className="flex gap-2">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline">
                  {interval.label} <ChevronDownIcon className="size-4 ml-2" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline">
                  {timezone} <ChevronDownIcon className="size-4 ml-2" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
...
async function ViewsByDealsCard(
  props: Parameters<typeof getViewsByDealsChartData>[0]
) {
  const chartData = await getViewsByDealsChartData(props);

  return (
    <Card>
      <CardHeader>
        <CardTitle>Visitors Per Deal Group</CardTitle>
      </CardHeader>
      <CardContent>
        <ViewsByDealsChart chartData={chartData} />
      </CardContent>
    </Card>
  );
}
