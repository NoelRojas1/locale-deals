...
export const productRelations = relations(ProductTable, ({ one, many }) => ({
  ...
  productViewTables: many(ProductViewTable),
  countryGroupDiscounts: many(CountryGroupDiscountTable),
}));

export const ProductCustomizationTable = pgTable("product_cutomizations", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  locationMessage: text("location_message")
    .notNull()
    .default(
      "Hey! It looks like you are from <strong>{country}</strong>. We support Parity Purchasing Power, so if you need it, use code <strong>'{coupon}'</strong> to get <strong>{discount}%</strong> off"
    ),
...
export const productViewTableRelations = relations(
  ProductViewTable,
  ({ one }) => ({
    product: one(ProductTable, {
