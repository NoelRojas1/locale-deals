...
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "product_cutomizations" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"class_prefix" text,
	"product_id" uuid NOT NULL,
	"location_message" text DEFAULT 'Hey! It looks like you are from <strong>{country}</strong>. We support Parity Purchasing Power, so if you need it, use code <strong>"{coupon}"</strong> to get <strong>{discount}%</strong> off' NOT NULL,
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "product_cutomizations_product_id_unique" UNIQUE("product_id")
);
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "product_cutomizations" ADD CONSTRAINT "product_cutomizations_product_id_products_id_fk" FOREIGN KEY ("product_id") REFERENCES "public"."products"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
